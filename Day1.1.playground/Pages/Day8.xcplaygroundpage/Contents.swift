...
for v in input.components(separatedBy: "\n") {
    let parts = v.components(separatedBy: " | ")
    let output = parts.last!.split(separator: " ").map { String($0) }
...
let result = outs.reduce(0) { next, value in
    return next + value.filter { e  in target.contains(e.count) }.count
}
